...
const Dashboard = () => {
    ...
    const fetchEnrollments = async (token) => {
        try {
            const response = await apiClient.get('/api/programs/my');
            if (response.status === 200) {
    ...
    const handleCancelBooking = async (bookingId) => {
        ...
        const token = localStorage.getItem('token');
        try {
            const response = await apiClient.delete(`/user/bookings/${bookingId}`);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await apiClient.put('/user/profile', { name: user.name, phone: user.phone, currency_preference: user.currency_preference });
